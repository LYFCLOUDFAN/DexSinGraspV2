...
class StateBasedGraspCustomed(BaseTask):
    ...
    def _create_objects(
        ...
        """
        为单个 env 实例化目标物体 + 若干干扰物体（全部来自 YCB 资产）
        ------------------------------------------------------------------
        * 第 0 个：目标物体（target）
        * 其余   ：干扰物体（clutter_i, i = 1..surrounding_obj_num）
        """
        # ------------------------------------------------------------
        # 常量：目标尺寸（最长边压缩到 8 cm）
        # ------------------------------------------------------------
        TARGET_SIZE = 0.015  # 改这里即可整体调大小 (单位: m)

        # ------------------------------------------------------------
        # 1. AssetOptions（与 _load_object_shelf_goal_assets 对齐）
        # ------------------------------------------------------------
        opt = gymapi.AssetOptions()
        opt.fix_base_link             = False
        opt.collapse_fixed_joints     = True
        opt.disable_gravity           = False
        opt.flip_visual_attachments   = False
        ...
        for i in range(self.surrounding_obj_num + 1):
            ...
            # ---------- 4.1 选择一个 YCB 物体 ----------
            if i == 0:
                ycb_info = random.choice(self.ycb_asset_list)  # 目标物体
                self._target_ycb_info_buffer[env_id] = ycb_info
            else:
                ycb_info = random.choice(self.ycb_asset_list)  # 干扰物体

            asset = get_asset(ycb_info["urdf"])

            # ---------- 4.2 计算缩放系数 (基于 mesh.extents) ----------
            max_dim = max(ycb_info["mesh"].extents)
            scale_factor = TARGET_SIZE / max_dim

            # ---------- 4.3 设置初始位姿 ----------
            pose = gymapi.Transform()
            pose.p.z = self.top_shelf_z + 0.01
            if i == 0:
                pose.p.x = pose.p.y = 0.0
            else:
                pose.p.x = random.uniform(-0.10, 0.10)
                pose.p.y = random.uniform(-0.10, 0.10)
            yaw = random.uniform(-3.1416, 3.1416)
            pose.r = gymapi.Quat().from_euler_zyx(0.0, 0.0, yaw)
            ...
            # ---------- 4.6 可选：随机质量 ----------
            if getattr(self, "randomize_object_mass", False):
                props = self.gym.get_actor_rigid_body_properties(env_ptr, handle)
                delta = random.uniform(-self.randomize_object_mass_range,
                                    self.randomize_object_mass_range)
                props[0].mass = max(0.01, props[0].mass + delta)
    ...
    def _load_object_shelf_goal_assets(self, assets_path, scale2str):
        ...
        object_start_pose.p     = gymapi.Vec3(0.0, 0.0, self.top_shelf_z + 0.02)
        object_start_pose.r     = gymapi.Quat().from_euler_zyx(0, 0, 0)

        # 目标点相对位移（沿 −x 方向 0.20 m）
