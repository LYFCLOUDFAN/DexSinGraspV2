...
class StateBasedGraspCustomed(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.object_scale_file == "Default":
            ...
            self.object_scale_list = [f"{n}/{s}" for n, scales in self.object_scale_dict.items() for s in scales]
            self.object_code_list  = list(self.object_scale_dict.keys())
        else:
            yaml_file = osp.join(BASE_DIR, f"results/configs/{self.object_scale_file}")
            (self.object_line_list,
            self.object_scale_list,
            self.object_scale_dict) = load_object_scale_result_yaml(
                                            yaml_file,
                                            self.start_line,
                                            self.end_line,
                                            self.shuffle_dict)
        ...
        self.random_time_flag = self.cfg["env"]["random_time"]   # 不覆盖 self.random_time
        ...
        goal_start_pose,
        table_pose) = \
            self._load_object_shelf_goal_assets(self.assets_path, self.scale2str)   # 这里 table 已被货架替代

        point_cloud_asset = self._create_point_asset()
        # 货架几何信息（在 _load_object_table_goal_assets 中已生成）
        # self.shelf_parts, self.shelf_poses, self.shelf_sizes, self.top_shelf_z

        table_texture_files  = osp.join(self.assets_path, "textures/texture_stone_stone_texture_0.jpg")
        table_texture_handle = self.gym.create_texture_from_file(self.sim, table_texture_files)

        # ------------------------------------------------------------------ #
        # 4. 预先申请常用 list / tensor（保持原样）
        # ------------------------------------------------------------------ #
        extra_aggre_num = 8
        ...
        self.object_id_buf = []      # 新增：记录环境对应的物体编号
        ...
        for env_id in tqdm.tqdm(range(num_envs), desc="Creating env"):
            ...
            # ----------------- 5.2  物体 & 目标球
            # 随机挑选 object_scale_name （与旧代码一致）
            if self.shuffle_env:
                temp_id = random.randint(0, len(self.object_scale_list) - 1)
            else:
                seg = num_envs // len(self.object_scale_list)
                if seg:
                    temp_id = env_id // seg
                else:
                    temp_id = int(env_id / num_envs * len(self.object_scale_list))
            self.env_object_scale_id.append(temp_id)

            object_scale_name = self.object_scale_list[temp_id]
            object_code = f"{object_scale_name.split('/')[0]}/{object_scale_name.split('/')[1]}"
            scale = float(object_scale_name.split('/')[2])
            scale_str = self.scale2str[scale]

            self.object_id_buf.append(env_id)
            self.object_scale_buf[env_id] = scale

            # 准备物体聚簇列表（目标物体 + 干扰物体）
            self.object_codes_in_clutter = list(self.object_asset_dict.keys())
            self.scale_strs_in_clutter   = [list(self.object_asset_dict[c].keys())[0] for c in self.object_codes_in_clutter]
            self.obj_focus_id = 0

            # 加载目标物体资产信息
            if self.use_object_asset_dict:
                object_asset_info = self.object_asset_dict[self.object_codes_in_clutter[0]][self.scale_strs_in_clutter[0]]
            else:
            ...
            # ---------- 记录目标物体初始状态（必须在 create_objects 之前准备好） ----------
            self.object_init_state.append([
                object_start_pose.p.x, object_start_pose.p.y, object_start_pose.p.z,
                object_start_pose.r.x, object_start_pose.r.y, object_start_pose.r.z, object_start_pose.r.w,
                0, 0, 0, 0, 0, 0
            ])
            # 保存目标物体的初始点云和 PCA 轴数据，避免空值错误
            mesh       = object_asset_info[2]   # Trimesh 对象
            pc_points  = object_asset_info[3]   # 表面点云 (numpy 数组)
            pc_centered= object_asset_info[4]   # 去中心的点云
            pca_axes   = object_asset_info[5]   # PCA 主轴 (3x3 矩阵)
            self.object_init_mesh['mesh'].append(mesh)
            self.object_init_mesh['mesh_vertices'].append(np.array(mesh.vertices))
            self.object_init_mesh['mesh_faces'].append(np.array(mesh.faces))
            self.object_init_mesh['points'].append(pc_points)
            self.object_init_mesh['points_centered'].append(pc_centered)
            self.object_init_mesh['pca_axes'].append(pca_axes)

            # ---------- 创建物体聚簇（保持原逻辑 _create_objects） ----------
            self.object_clutter_handles = []
            self.env_id = env_id
            self._create_objects(env_ptr, env_id,
                                self.object_codes_in_clutter,
                                self.scale_strs_in_clutter,
                                type=2)
            # 获取目标物体的句柄和索引
            ...
            self.gym.set_actor_scale(env_ptr, goal_handle, 0.0001)  # 缩放目标球，使其几乎不可见
            ...
            for part_asset, part_pose in zip(self.shelf_parts, self.shelf_poses):
                sh_actor = self.gym.create_actor(
                    ...
                    "shelf_part", env_id - 5 * self.num_envs, -1, SEGMENT_ID['table'][0])  # 使用 table 类别ID
                shelf_handles.append(sh_actor)
                self.table_indices.append(self.gym.get_actor_index(env_ptr, sh_actor, gymapi.DOMAIN_SIM))

            # ---------- 统一设置货架板的材质 / 摩擦 / 颜色 ----------
            for i, sh in enumerate(shelf_handles):
                # 可选：为第一块板贴纹理（如不需要可删除）
                # if i == 0:
                #     self.gym.set_rigid_body_texture(env_ptr, sh, 0, gymapi.MESH_VISUAL, table_texture_handle)
                shape_props = self.gym.get_actor_rigid_shape_properties(env_ptr, sh)
                for prop in shape_props:
                    prop.friction = 1.0
                self.gym.set_actor_rigid_shape_properties(env_ptr, sh, shape_props)
                # 设置为灰色外观
                self.gym.set_rigid_body_color(env_ptr, sh, 0, gymapi.MESH_VISUAL,
                                            gymapi.Vec3(150/255, 150/255, 150/255))

            # ---------- 5.4  点云可视化 / 颜色调整（物体不变） ----------
            # -- 以下逻辑与原代码一致，可根据需要添加 --
            # （此处根据原始实现，可使用 point_cloud_asset 在目标物体表面创建点云可视化，也可设置物体/机械手的颜色等。
            #  注意确保没有对 table_handle 的引用，将其替换为 shelf_handles 或跳过相关步骤）
            # ...
        ...
        if self.num_cameras > 0:
            self.view_mat    = torch.stack([torch.stack(mats) for mats in self.camera_view_mat_list])
            self.vinv_mat    = torch.stack([torch.stack(mats) for mats in self.camera_vinv_mat_list])
            self.proj_matrix = torch.stack([torch.stack(mats) for mats in self.camera_proj_mat_list])

        # -------- 将列表数据转换为 Torch 张量以用于并行环境 --------
        ...
        self.object_id_buf = to_torch(np.array(self.object_id_buf), dtype=torch.long, device=self.device)
    ...
    def _create_objects(self, env_ptr, env_id, object_codes_in_clutter, scale_strs_in_clutter,type=1):
        ...
        for i in range(self.surrounding_obj_num+1):
            ...
            object_start_pose_i.p.z = self.top_shelf_z + 0.01
    ...
    def _create_shelf_assets(self):
        ...
        """
        n_levels      = 3
        board_thick   = 0.02          # 2 cm 厚
        board_width   = 0.80          # x 方向 40 cm
        board_depth   = 0.60          # y 方向 30 cm
        gap_between   = 0.30          # 层间距 15 cm

        parts, poses, sizes = [], [], []
        for lvl in range(n_levels):
            z_center = self.table_dims.z + board_thick/2 + lvl*(board_thick + gap_between)

            # 1) asset（所有层板可以复用同一个 asset）
            if lvl == 0:
                board_asset_opt = gymapi.AssetOptions()
                board_asset_opt.fix_base_link = True
                board_asset = self.gym.create_box(
                    self.sim, board_width, board_depth, board_thick, board_asset_opt
                )
            # 后续层板直接复用同个 asset
            ...
            pose.p = gymapi.Vec3(0.0, 0.0, z_center)
            poses.append(pose)

            # 3) extents
            sizes.append(gymapi.Vec3(board_width, board_depth, board_thick))

        top_shelf_z = poses[1].p.z + board_thick / 2
        # import ipdb;ipdb.set_trace()
    ...
    def _load_object_shelf_goal_assets(self, assets_path, scale2str):
        """
        1. 读取所有可抓取 Object 的 asset（基本保持原逻辑）
        2. 创建目标球 asset
        3. 调用 _create_shelf_assets() 生成货架多个板子
        4. 计算初始摆放位姿与渲染用的 mesh
        5. 返回 6 个值，与原来接口一致
        """

        # ----------------------------------------------------------------------------------
        # A. 加载 Object asset（保留原来逻辑，只微调打印格式）
        # ----------------------------------------------------------------------------------
        object_asset_dict = {}
        self.use_object_asset_dict = len(self.object_scale_list) <= 500
        ...
        self.object_asset_options.vhacd_params.resolution = 300_000
        self.object_asset_options.default_dof_drive_mode  = gymapi.DOF_MODE_NONE

        object_asset = None
        self.num_object_bodies  = -1
        self.num_object_shapes = -1

        for object_id in tqdm.tqdm(range(len(self.object_code_list)), desc="Loading object meshes"):
            object_code = self.object_code_list[object_id]
            if not self.use_object_asset_dict and object_id != 0:
                continue

            object_asset_dict[object_code] = {}
            for scale in self.object_scale_dict[object_code]:
                scale_str = scale2str[scale]
                object_asset_info = self._load_object_asset_info(
                    assets_path, object_code, scale_str
                )
                object_asset_dict[object_code][scale_str] = object_asset_info

                # 统计 b odies / shapes 个数
                self.num_object_bodies = max(
                    self.num_object_bodies,
                    self.gym.get_asset_rigid_body_count(object_asset_info[0]),
                )
                self.num_object_shapes = max(
                    self.num_object_shapes,
                    self.gym.get_asset_rigid_shape_count(object_asset_info[0]),
                )
                if object_asset is None:
                    object_asset = object_asset_info[0]
        ...
        # 货架不是单一 asset，下面的 table_asset / table_pose 留空（保持返回六元组接口）
        table_asset = None
        table_pose  = None
        ...
        object_start_pose.p     = gymapi.Vec3(0.0, 0.0, self.top_shelf_z + 0.05)  # 顶层上方 2 cm
        object_start_pose.r     = gymapi.Quat().from_euler_zyx(0, 0, 0)

        self.goal_displacement  = gymapi.Vec3(-0.20, 0.0, 0.0)
        ...
        # ----------------------------------------------------------------------------------
        # E.  将货架 mesh 保存到渲染用 tensor（替代原 table_xxx 变量）
        # ----------------------------------------------------------------------------------
        import trimesh, numpy as np

        shelf_verts, shelf_faces  = [], []
        vert_offset = 0
        for part_pose, extents in zip(self.shelf_poses, self.shelf_sizes):
            mesh = trimesh.creation.box(
                extents=(extents.x, extents.y, extents.z)
            )
            mesh.apply_translation(
                [part_pose.p.x, part_pose.p.y, part_pose.p.z]
            )
