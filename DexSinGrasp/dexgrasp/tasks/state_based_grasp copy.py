...
class StateBasedGrasp(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        self.num_agents = 1
        self.cfg["env"]["numActions"] = 22 # + 6 # 6 is for the arm
        # # Device params
        ...
        self.shadow_hand_default_dof_pos = torch.tensor(self.init_dof_state, dtype=torch.float, device=self.device)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        max_agg_bodies = self.num_shadow_hand_bodies * 1 + 2 * self.num_object_bodies + 1 +extra_aggre_num##
        max_agg_shapes = self.num_shadow_hand_shapes * 1 + 20 * self.num_object_shapes + 1 +extra_aggre_num ##
        ...
        for env_id in loop:
            ...
            shadow_hand_actor = self.gym.create_actor(env_ptr, shadow_hand_asset, shadow_hand_start_pose, "hand", env_id - 5*self.num_envs, -1, SEGMENT_ID['object'][0])
            ...
            shadow_hand_dof_props["driveMode"][:6] = gymapi.DOF_MODE_POS
            shadow_hand_dof_props["driveMode"][6:] = gymapi.DOF_MODE_POS
            shadow_hand_dof_props["stiffness"] = [50] * 2 + [150] * 1 + [100]*3 + [100] * 16
            shadow_hand_dof_props["armature"] = [0.001] * 6 + [0.0001] * 16 # NOTE: magic!  0.001, 0.0001
            shadow_hand_dof_props["damping"] = [20] * 6 + [5] * 16 # [5] *16 when training

            self.gym.set_actor_dof_properties(env_ptr, shadow_hand_actor, shadow_hand_dof_props)
            hand_idx = self.gym.get_actor_index(env_ptr, shadow_hand_actor, gymapi.DOMAIN_SIM)
            self.hand_indices.append(hand_idx)

            # # NOTE force control for arm and position control for fingers
            # props = self.gym.get_actor_dof_properties(env_ptr, shadow_hand_actor)
            
            # self.gym.set_actor_dof_properties(env_ptr, shadow_hand_actor, props)
            ...
            # NOTE: load clutters, instead of single object
            ...
            ################# customed object type and surrounding type
            self.env_id = env_id
    ...
    def _create_objects(self, env_ptr, env_id, object_codes_in_clutter, scale_strs_in_clutter,type=1):
        ...
        # object_1x1_asset1 = self.gym.create_box(self.sim, side_len, side_len, side_len, object_asset_options)
        ...
        for i in range(self.surrounding_obj_num+1):
            ...
            if i == 0 and self.expert_id != 3:
                ...
                # # object_props[0].mass = 0.2 # default 0.7 kg for 0.1 scale, 0.36 kg for 0.08 scale
                # print(object_props[0].mass)
                # exit()
                # self.gym.set_actor_rigid_body_properties(env_ptr,object_handle_i,object_props,recomputeInertia=True)
                # object_handle_i = self.gym.create_actor(env_ptr, object_1x1_asset, object_start_pose_i, "object_i", env_id, 0, SEGMENT_ID['object'][0])
                ...
                if self.expert_id == 1:
                    ...
                    if rand_val < 0.75: # 0.85 used for training, 0.75 used for test
                        object_handle_i = self.gym.create_actor(env_ptr, object_1x1_asset, object_start_pose_i, "object_i", env_id - 5*self.num_envs, 0, SEGMENT_ID['object'][0])
                    elif rand_val < 0.95: # 0.98 used for training, 0.95 used for test
                ...
                if self.randomize_object_mass:
                    ...
                    object_props[0].mass += random.uniform(-self.randomize_object_mass_range, self.randomize_object_mass_range)
                    self.gym.set_actor_rigid_body_properties(env_ptr, object_handle_i, object_props, recomputeInertia=True)
            # object_rb_props = self.gym.get_actor_rigid_body_properties(env_ptr, object_handle_i)
            # print("object mass",object_rb_props[0].mass)
                
                # object_props = self.gym.get_actor_rigid_body_properties(env_ptr, object_handle_i)
                # object_props[0].mass = 0.2# default 0.7 
                # self.gym.set_actor_rigid_body_properties(env_ptr,object_handle_i,object_props,recomputeInertia=True)
            # else:
            #     object_handle_i = self.gym.create_actor(env_ptr, object_1x1_asset, object_start_pose_i, "object_i", 10*env_id+1, 0, SEGMENT_ID['object'][0])
            # self.gym.set_actor_scale(env_ptr, object_handle_i, self.object_scale_singulation)
    ...
    def _load_shadow_hand_assets(self, assets_path):        
        shadow_hand_asset_file = "xarm6/dummy_leap_vertical_moving.urdf"
        ...
        asset_options.angular_damping = 100 #100
        asset_options.linear_damping = 100 #100
        if self.physics_engine == gymapi.SIM_PHYSX:
            asset_options.use_physx_armature = True
        asset_options.default_dof_drive_mode = gymapi.DOF_MODE_POS # NOTE to be changed outside the function
        ...
        self.valid_shadow_hand_bodies = [i for i in range(8,self.num_shadow_hand_bodies)]


        # print rigid body names and indices
        # print("\nShadow hand rigid body names and indices:")
        # for i in range(self.num_shadow_hand_bodies):
        #     print(f"{i}: {self.gym.get_asset_rigid_body_name(shadow_hand_asset, i)}")
        ...
        if 'static_init' in self.config['Modes'] and self.config['Modes']['static_init']:
            ...
            shadow_hand_start_pose.p = gymapi.Vec3(hand_shift_x, hand_shift_y, self.table_dims.z + hand_shift_z)  # gymapi.Vec3(0.1, 0.1, 0.65)
            ...
            shadow_hand_start_pose = gymapi.Transform()
            shadow_hand_start_pose.p = gymapi.Vec3(0.0, 0.0, self.table_dims.z + 0.3)  # gymapi.Vec3(0.1, 0.1, 0.65)
            shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(0, 0, 0)  # gymapi.Quat().from_euler_zyx(0, -1.57, 0)
    ...
    def _load_object_asset_info(self, assets_path, object_code, scale_str):
        ...
        # scaled_object_points,scaled_object_pc_fps
        # NOTE
    ...
    def render_camera_point_clouds(self, depth_tensor, seg_tensor, vinv_mat, proj_matrix, render_scene_only=True):
        ...
        self.step_count += 1
        threshold = [10,80,85,90,95,100,105,110,115,120,125,130,135,140,145,150]
        if self.step_count > 130:
            exit()
        if self.step_count in threshold:
            # Visualize points using open3d
            import open3d as o3d
            import numpy as np
            import PIL.Image
            
            # Extract points and segment IDs from first environment and convert to numpy array
            points_np = points_batch[2,:,:3].detach().cpu().numpy()  # XYZ coordinates
            segment_ids = points_batch[2,:,3].detach().cpu().numpy() # Segment IDs
            
            # Create colors based on segment IDs
            colors = np.zeros_like(points_np)
            colors[segment_ids == SEGMENT_ID['hand'][0]] = [1, 0, 0]    # Red for hand
            colors[segment_ids == SEGMENT_ID['object'][0]] = [0, 1, 0]  # Green for object
            ...
            # Function to capture and save view
            def save_view(vis, filename, front, up, zoom=1.1):
                ctr = vis.get_view_control()
                ctr.set_front(front)
                ctr.set_up(up)
                ctr.set_lookat([0, 0, 0.6])
                ctr.set_zoom(zoom)
                ...
                img_rgba = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=-1)
                img_rgba[np.all(img == [255, 255, 255], axis=-1)] = [255, 255, 255, 0]
                PIL.Image.fromarray(img_rgba).save(filename)

            # Set up visualization
            vis = o3d.visualization.Visualizer()
            vis.create_window(width=1000, height=1000, visible=False)
            
            # Set white background and point size
            ...
            save_view(vis, "pc_view_top_{}.png".format(self.step_count), [0, 0, 1], [0, 1, 0])  # Top view
            save_view(vis, "pc_view_front_{}.png".format(self.step_count), [0, -1.5, 0.8], [0, 0, 1])  # Front view
        '''
    ...
    def pre_physics_step(self, actions):
        ...
        #     f.write('\n')
        #     f.write(f"======Action {self.step_count}==========\n")
        #     f.write(f"actions dim {actions.shape}\n")
        #     f.write(f"{actions}\n")
        ...
        # finger joints
        self.cur_targets[:, self.actuated_dof_indices[6:]] = scale(self.actions[:, 6:],self.shadow_hand_dof_lower_limits[self.actuated_dof_indices[6:]],self.shadow_hand_dof_upper_limits[self.actuated_dof_indices[6:]])
        ...
        # hand translation
        targets = self.prev_targets[:, self.actuated_dof_indices[:3]] + self.shadow_hand_dof_speed_scale * self.dt * self.actions[:, :3] 
        self.cur_targets[:, self.actuated_dof_indices[:3]] = tensor_clamp(targets, self.shadow_hand_dof_lower_limits[self.actuated_dof_indices[:3]],self.shadow_hand_dof_upper_limits[self.actuated_dof_indices[:3]])

        # hand rotation
        if self.use_hand_rotation:
            targets = self.prev_targets[:, self.actuated_dof_indices[3:6]] + self.shadow_hand_dof_speed_scale * self.dt * self.actions[:, 3:6] * self.hand_rotation_coef   
            self.cur_targets[:, self.actuated_dof_indices[3:6]] = tensor_clamp(targets, self.shadow_hand_dof_lower_limits[self.actuated_dof_indices[3:6]],self.shadow_hand_dof_upper_limits[self.actuated_dof_indices[3:6]])
        ...
        # real world pose for pipeline figure
        # print(self.step_count)
        # self.step_count += 1
        # if self.step_count >= 300 + 80:
        #     print(self.cur_targets[:, self.actuated_dof_indices[6:]])
        #     time.sleep(100)
    ...
    def post_physics_step(self):
        ...
        if self.viewer:
            if self.debug_viz:
                ...
                for i in range(self.num_envs):
                    ...
                    # self.add_debug_vector(self.envs[i], self.hand_body_pos[0], self.actions[i, :3]*10,color=[0.1, 0.85, 0.1])
                    # self.add_debug_vector(self.envs[i], self.right_hand_pos[i], 1000*(self.right_hand_pos[i]-self.prev_right_hand_pos[i]),color=[0.85, 0.1, 0.1])

                    # NOTE: object pc_fps and object mesh
                    # import ipdb; ipdb.set_trace()
                    # for k in range(0, (self.object_init_mesh['mesh_vertices'].shape[1]), 1):

                    #     self.add_debug_lines(self.envs[i], to_torch([self.object_init_mesh['mesh_vertices'][i][k][0],
                    #                                                  self.object_init_mesh['mesh_vertices'][i][k][1]+0.2,
                    #                                                  self.object_init_mesh['mesh_vertices'][i][k][2]+0.75] , device=self.device), self.object_rot[i].to(dtype=torch.float32))
                    # for k in range(0, (self.object_init_mesh['points'].shape[1]), 20):

                    #     self.add_debug_lines(self.envs[i], to_torch([self.object_init_mesh['points'][i][k][0],
                    #                                                  self.object_init_mesh['points'][i][k][1]-0.2,
                    #                                                  self.object_init_mesh['points'][i][k][2]+0.75] , device=self.device), self.object_rot[i].to(dtype=torch.float32))
        ...
        if self.config['Save_Render'] or self.sample_point_clouds:
            ...
            # # Extract points from first environment and convert to numpy array
            # # print(self.step_count)
            # print(points_fps.shape)
            # self.step_count += 1
            # points_np = points_fps[0,:,:3].detach().cpu().numpy()
            # # Save points to numpy file
            # if not hasattr(self, 'saved_points'):
            #     self.saved_points = {}
            # self.saved_points[self.step_count] = points_np
            # np.save(f'points_{self.step_count}.npy', points_np)
            ...
            # # Save figure
            # plt.savefig(f'point_cloud_{self.step_count}.png')
            # plt.close()
            # import ipdb; ipdb.set_trace()
            # print(self.pytorch_rendered_points.shape)
            ...
            # Save rendered object points to file
            # if not hasattr(self, 'saved_rendered_points'):
            #     self.saved_rendered_points = {}
            # self.saved_rendered_points[self.frame] = self.rendered_object_points.detach().cpu().numpy()
            # # Save points to file
            # np.save(f'rendered_points_{self.frame}.npy', self.rendered_object_points.detach().cpu().numpy())

            #print(self.rendered_object_points.shape)
    ...
    def compute_observations(self):
        ...
        # # # Visualize object points using Open3D
        # import open3d as o3d
        # # Print points_centered limits and ranges
        # points = self.object_init_mesh['points'][0]
        # print("\nObject points limits and ranges:")
        # print(f"X range: {points[:,0].min():.3f} to {points[:,0].max():.3f}, span: {points[:,0].max() - points[:,0].min():.3f}")
        # print(f"Y range: {points[:,1].min():.3f} to {points[:,1].max():.3f}, span: {points[:,1].max() - points[:,1].min():.3f}") 
        # print(f"Z range: {points[:,2].min():.3f} to {points[:,2].max():.3f}, span: {points[:,2].max() - points[:,2].min():.3f}")
        # # Convert points to numpy array and create Open3D point cloud
        # points_np = self.object_init_mesh['points_centered'][0].cpu().numpy() # Take first environment's points
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(points_np)
        ...
        # {'wrist': -1, 'palm': 8, 'thumb': 17, 'index': 12, 'middle': 22, 'ring': 27}
        ...
        # TODO fix this, stays constant at the time
        ...
        if self.vision_based:
            ...
            # import open3d as o3d
            # # Print points_centered limits and ranges
            # points = self.rendered_object_points
            # print("\nObject points limits and ranges:")
            # print(f"X range: {points[:,0].min():.3f} to {points[:,0].max():.3f}, span: {points[:,0].max() - points[:,0].min():.3f}")
            # print(f"Y range: {points[:,1].min():.3f} to {points[:,1].max():.3f}, span: {points[:,1].max() - points[:,1].min():.3f}") 
            # print(f"Z range: {points[:,2].min():.3f} to {points[:,2].max():.3f}, span: {points[:,2].max() - points[:,2].min():.3f}")
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        # print("hand_euler_xyz", hand_euler_xyz)
        ...
        '''
        # original code to keep up with randomized gird sequence
        ...
        # Loop through all other objects (indices 0-8 except target object index) 
        for i in range(self.surrounding_obj_num+1):
            if i != self.obj_focus_id:
                ...
                grid_positions = self.object_position_assignments[:, i]  # Get grid position for object i
                init_object_pos = torch.stack([
                    self.separation_dist * torch.tensor([self.sudoku_grid[pos.item()]['x'] for pos in grid_positions], 
                                                    device=proj_dist.device, dtype=proj_dist.dtype),
                    self.separation_dist * torch.tensor([self.sudoku_grid[pos.item()]['y'] for pos in grid_positions], 
                                                    device=proj_dist.device, dtype=proj_dist.dtype)
                ], dim=1)
        ...
        # accelerated version (avoid frequent list operations)
        # Calculate singulation distances for target object
        object_distances = []
        projected_distances = []
        singulation_distances = []
        ...
        # obs_dict['hand_dofs'][:,0:6] = 0  # dummy joint pos
        # obs_dict['hand_dofs'][:,22:28] = 0  # all joint vel # 这个信息很关键
        # obs_dict['hand_dofs'][:,44:66] = 0  # all joint force
        # obs_dict['objects'][:,-13:] = 0  # object last 13 dim 
        obs_dict['singulation_distances'][:] = 0  # singulation distances

        # obs_dict['times'][:] = 0         #时间，因为频率不同可能有害于sim2real,有时间的都要扔掉，
        # obs_dict['actions'][:] = 0           #上一次的action， 手指部分是 smoothed_action = 0.1 * action + 0.9 * last_action, 和时间无关， 但是手掌部分的dummy joint action 是delta action 和时间有关，最好去掉
        #                                 # 如果不能扔掉，我们还需测试smoothed_action 在xarm是否可以跑通，尽可能与时间无关 (其他paper里面arm是带delta的)

        # obs_dict['hand_objects'][:,:] = 0    #if self.vision_based: 按照未分割的点云来取平均值来近似等于object_centers
        # obs_dict['object_visual'][:] = 0
        # print(self.step_count)
        # self.step_count += 1
        # Cat Final Obs Buff
        # print(obs_dict['objects'][0])
        self.obs_buf = torch.cat([obs_dict[name] for name in self.obs_names if name in obs_dict], dim=-1)
        # with open('obs.txt', 'a') as f:
        #     f.write(f"===========================iteration {self.step_count}===============================\n")
        #     f.write("Observation dimensions:\n")
        #     for name in self.obs_names:
        #         if name in obs_dict:
        #             f.write(f"{name}: {obs_dict[name]}\n")
        #     f.write(f"Final concatenated observation shape: {self.obs_buf.shape}\n")
        #     f.write(str(self.obs_buf))
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        self.reset_dof_state = self.dof_state.clone().view(self.num_envs, self.num_shadow_hand_dofs,-1)
        ...
        if 'static_init' in self.config['Modes'] and self.config['Modes']['static_init']:
            ...
            # target_pos_rot[:, :2] += torch_rand_float(-0.13, 0.13, (target_pos_rot.shape[0], 2), device=target_pos_rot.device.type)  # random xy position
            ...
            # rand_pos = self.separation_dist  + 0.025 * torch.rand(1, device=self.device)
...
#####################################################################
@torch.jit.script
def compute_pure_singulation_reward(
    ...
    mean_singulation_distance, min_singulation_distance, table_z: float, invalid_env_mask, invalid_env_num: int
):
    # # ---------------------- State Update  ---------------------- # #
    # Action penalty
    ...
    singulation_reward = 15*singulation_reward


    singulation_bonus = torch.where((min_singulation_distance >= 0.01) & (right_hand_dist <= 0.03), 
                                     min_singulation_distance * 50, 
                                     torch.zeros_like(min_singulation_distance))

    # # ---------------------- Total Reward ---------------------- # #
    init_reward = weights['right_hand_dist'] * right_hand_dist
    init_reward += weights['right_hand_joint_dist'] * right_hand_joint_dist

    reward = init_reward + singulation_reward + singulation_bonus
    ...
    # Compute successes: reach singulation criterion
    successes = torch.where((min_singulation_distance >= 0.005) & (right_hand_dist <= 0.03), torch.ones_like(successes), successes)
    # Remove successes in invalid environments (object burst away during init)
    ...
    final_successes = torch.where(min_singulation_distance >= 0.02, torch.ones_like(successes), torch.zeros_like(successes))
...
@torch.jit.script
def compute_grasping_only_reward(
    modes: Dict[str, bool], weights: Dict[str, float],
    object_init_z, delta_target_hand_pos, delta_target_hand_rot,
    id: int, object_id, dof_pos, rew_buf, reset_buf, reset_goal_buf, progress_buf,
    successes, current_successes, consecutive_successes,
    ...
    # print('============')
    # print("max_hand_dist, max_finger_dist", max_hand_dist, max_finger_dist)
    # print("singulation_reward", singulation_reward.mean())
    # print("total reward", reward.mean())
    # print("hand_up", hand_up.mean())
    # print('lowest', lowest.mean())
    # print('--')
    # print("right_hand_dist.mean(), right_hand_joint_dist.mean(), right_hand_finger_dist.mean(), hold_flag[0:10], successes\n[ CUDAFloatType{} ]",
    #         right_hand_dist.mean(),
    #         right_hand_joint_dist.mean(),
    #         right_hand_finger_dist.mean(),
    #         hold_flag[0:5],
    #         torch.sum(successes))
...
def compute_hand_reward(
    ...
    # print(right_hand_joint_dist.mean(),right_hand_dist.mean())
