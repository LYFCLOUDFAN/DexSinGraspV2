...
class XArmAllegroHandCubeInBox(XArmAllegroHandFunctionalManipulationUnderarm):

    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        # Read/override environment options
        env_cfg = cfg["env"]
    ...
    def compute_aggregate_bodies_and_shapes(self, env: int, gym_assets: Optional[Dict] = None) -> Tuple[int, int]:
        ...
        # floor(1) + wall_y(2) + wall_x(2) = 5 actors
        # Multiply counts accordingly
        ...
        n_rb_y = self.gym.get_asset_rigid_body_count(cont["assets"]["wall_pos_y"])
        n_rs_y = self.gym.get_asset_rigid_shape_count(cont["assets"]["wall_pos_y"])
        n_rb_x = self.gym.get_asset_rigid_body_count(cont["assets"]["wall_pos_x"])
        n_rs_x = self.gym.get_asset_rigid_shape_count(cont["assets"]["wall_pos_x"])
    ...
    def _create_envs(self, num_envs: int, spacing: float, num_objects_per_env: int = 1):
        print(">>> Setting up %d environments (CubeInBox)" % num_envs)
        ...
        for i in range(num_envs):
            ...
            h = self._create_sim_actor(env, {"asset": cont["floor"], "name": "container_floor",
                                              "num_rigid_bodies": 1, "num_rigid_shapes": 1,
                                              "pose": floor_pose}, -1, actor_handle=False, color=gymapi.Vec3(0.2, 0.2, 0.2))
            container_handles.append(h)
            ...
            h = self._create_sim_actor(env, {"asset": cont["wall_pos_y"], "name": "container_wall_pos_y",
                                              "num_rigid_bodies": 1, "num_rigid_shapes": 1,
                                              "pose": wall_posy_pose}, -1, actor_handle=False, color=gymapi.Vec3(0.25, 0.25, 0.25))
            container_handles.append(h)
            ...
            h = self._create_sim_actor(env, {"asset": cont["wall_neg_y"], "name": "container_wall_neg_y",
                                              "num_rigid_bodies": 1, "num_rigid_shapes": 1,
                                              "pose": wall_negy_pose}, -1, actor_handle=False, color=gymapi.Vec3(0.25, 0.25, 0.25))
            container_handles.append(h)
            ...
            h = self._create_sim_actor(env, {"asset": cont["wall_pos_x"], "name": "container_wall_pos_x",
                                              "num_rigid_bodies": 1, "num_rigid_shapes": 1,
                                              "pose": wall_posx_pose}, -1, actor_handle=False, color=gymapi.Vec3(0.25, 0.25, 0.25))
            container_handles.append(h)
            ...
            h = self._create_sim_actor(env, {"asset": cont["wall_neg_x"], "name": "container_wall_neg_x",
                                              "num_rigid_bodies": 1, "num_rigid_shapes": 1,
                                              "pose": wall_negx_pose}, -1, actor_handle=False, color=gymapi.Vec3(0.25, 0.25, 0.25))
            container_handles.append(h)
        ...
        print(f">>> Done creating {num_envs} environments (CubeInBox)")

    # ---------- Override done logic to mark success when cube is lifted out ----------
    # def compute_done(self, is_success):
    #     # Combine upstream 'is_success' with success-by-lift-out-of-box
    #     lifted = self.object_root_positions[:, 2] > (self._container_rim_z + self.cube_out_clearance)
    #     is_grasped = self.picked  # base logic for picked (height-based) serves as grasp indicator
    #     is_success_box = lifted & is_grasped

    #     combined_success = torch.where(is_success > 0, torch.ones_like(is_success), is_success_box.to(is_success.dtype))

    #     if not False:  # not test_sim path
    #         if self.env_mode == "pgm":
    #             fall_env_mask = (self.object_root_positions[:, 2] < self._table_pose_tensor[2] - 0.1)
    ...
    #         else:
    #             fall_env_mask = (self.object_root_positions[:, 2] < self.allegro_hand_center_positions[:, 2] - 0.2)
    #             arm_contact_mask = (self.arm_contact_forces.norm(p=2, dim=2) > 1).any(dim=1)
    #             failed_env_ids = (fall_env_mask | arm_contact_mask).nonzero(as_tuple=False).squeeze(-1)

    #         if self.success_steps > 0:
    #             self.progress_buf = torch.where(combined_success > 0, torch.zeros_like(self.progress_buf), self.progress_buf)
    #             self.reset_buf = torch.where(combined_success > 0, 1, self.reset_buf)

    #         self.reset_buf = torch.where(self.progress_buf >= self.max_episode_length - 1, 1, self.reset_buf)
    #         self.reset_buf[failed_env_ids] = 1

    #     # success bookkeeping
    #     succ_env_ids = (combined_success > 0).nonzero(as_tuple=False).squeeze(-1)
    ...
    #     self.extras["cube_out_success"] = (combined_success > 0).float()
    #     self.extras["container_rim_z"] = self._container_rim_z.clone()
